...
class Minerva_Triple_Api(object):
    ...
    def init_test_env(self, params):
        self.test_environment = env(params, 'test')
    ...
    def initialize_train(self, restore=None, sess=None):
        ...
        if not restore:
            ...
            logger.info("Succeeded in restoring model: {}".
                        format(restore))
            return  self.model_saver.restore(sess, restore)
        
    def initialize_test(self, restore=None, sess=None):
    ...
    def initialize_infer(self, restore=None, sess=None):
        # Building the test graph
        self.prev_state = tf.placeholder(tf.float32, self.agent.get_mem_shape(), name="memory_of_agent")
        self.prev_relation = tf.placeholder(tf.int32, [None, ], name="previous_relation")
        self.query_embedding = tf.nn.embedding_lookup(self.agent.relation_lookup_table, self.query_relation)  # [B, 2D]
        ...
        with tf.variable_scope("policy_steps_unroll") as scope:
            #scope.reuse_variables()
            self.infer_loss, infer_state, self.infer_logits, self.infer_action_idx, self.chosen_relation = self.agent.step(
                self.next_relations, self.next_entities, formated_state, self.prev_relation, self.query_embedding,
                self.current_entities, self.input_path[0], self.range_arr, self.first_state_of_test)
            self.infer_state = tf.stack(infer_state)
    ...
    def test(self, sess, beam=False, print_paths=False, save_model = True, auc = False):
        ...
        for episode in tqdm(self.test_environment.get_episodes()):
            ...
            for b in range(temp_batch_size):
                ...
                if answer_pos == None:
                    ...
                    AP += 1.0/((answer_pos+1))
                if print_paths:
                    qr = self.test_environment.grapher.rev_relation_vocab[self.qr[b * self.test_rollouts]]
        ...
        if print_paths:
            ...
            with codecs.open(self.path_logger_file_, 'w', 'utf-8') as pos_file:
                for q in paths:
                    for p in paths[q]:
                        pos_file.write(p)

        with open(self.output_dir + '/scores.txt', 'w') as score_file:
    ...
    def infer(self, sess, beam=False, print_paths=False, save_model = True, auc = False, input_triples=None):
        batch_counter = 0
        paths = defaultdict(list)
        feed_dict = {}
        auc = 0

        for episode in tqdm(self.infer_environment.get_episodes(input_triples)):
            batch_counter += 1
            ...
            feed_dict[self.query_relation] = self.qr
            # set initial beam probs
            beam_probs = np.zeros((temp_batch_size * self.infer_rollouts, 1))
            # get initial state
            state = episode.get_state()
            mem = self.agent.get_mem_shape()
            agent_mem = np.zeros((mem[0], mem[1], temp_batch_size*self.infer_rollouts, mem[3]) ).astype('float32')
            previous_relation = np.ones((temp_batch_size * self.infer_rollouts, ), dtype='int64') * self.relation_vocab[
                'DUMMY_START_RELATION']
            feed_dict[self.range_arr] = np.arange(temp_batch_size * self.infer_rollouts)
            feed_dict[self.input_path[0]] = np.zeros(temp_batch_size * self.infer_rollouts)
            ...
            self.log_probs = np.zeros((temp_batch_size*self.infer_rollouts,)) * 1.0
            ...
            for i in range(self.path_length):
                ...
                loss, agent_mem, infer_scores, infer_action_idx, chosen_relation = sess.run(
                    [ self.infer_loss, self.infer_state, self.infer_logits, self.infer_action_idx, self.chosen_relation],
                    feed_dict=feed_dict)


                if beam:
                    k = self.infer_rollouts
                    new_scores = infer_scores + beam_probs
                    if i == 0:
                        idx = np.argsort(new_scores)
                        idx = idx[:, -k:]
                        ranged_idx = np.tile([b for b in range(k)], temp_batch_size)
                ...
                if print_paths:
                    ...
                    self.relation_trajectory.append(chosen_relation)
                ####################
                state = episode(infer_action_idx)
                self.log_probs += infer_scores[np.arange(self.log_probs.shape[0]), infer_action_idx]
            if beam:
                self.log_probs = beam_probs
            ...
            self.log_probs = np.reshape(self.log_probs, (temp_batch_size, self.infer_rollouts))
            sorted_indx = np.argsort(-self.log_probs)
            
            ce = episode.state['current_entities'].reshape((temp_batch_size, self.infer_rollouts))
            for b in range(temp_batch_size):
                answer_pos = None
                seen = set()
                pos=0
                ...
                if print_paths:
                    qr = self.infer_environment.grapher.rev_relation_vocab[self.qr[b * self.infer_rollouts]]
                    start_e = self.rev_entity_vocab[episode.start_entities[b * self.infer_rollouts]]
                    end_e = self.rev_entity_vocab[episode.end_entities[b * self.infer_rollouts]]
                    paths[str(qr)].append(str(start_e) + "\n")
                    #paths[str(qr)].append("Reward:" + str(1 if answer_pos != None and answer_pos < 10 else 0) + "\n")
                    for r in sorted_indx[b]:
                        indx = b * self.infer_rollouts + r
    ...
    def train_api(self, params):
        ...
        with tf.Session(config=config) as sess:
            ...
            self.init_train_env(params)
            self.init_dev_env(params)
            self.initialize()
            sess.run(self.initialize_train())
            sess.run(self.initialize_test())
            self.initialize_pretrained_embeddings(sess=sess)
            self.train(sess)
            
    def infer_api(self, params, input_triples, restore=None, sess=None):
        config = tf.ConfigProto()
        config.gpu_options.allow_growth = False
        config.log_device_placement = False
        with tf.Graph().as_default():
            with tf.Session(config=config) as sess:
                self.agent = Agent(params)
                self.init_infer_env(params)
                self.initialize()
                self.initialize_infer(restore=restore, sess=sess)
                self.infer_rollouts = 50
                paths = self.infer(sess, beam=True, print_paths=True, save_model=False, input_triples=input_triples)
            return paths
    
    def test_api(self, params, restore=None, sess=None):
        config = tf.ConfigProto()
        config.gpu_options.allow_growth = False
        config.log_device_placement = False
        with tf.Graph().as_default():
            with tf.Session(config=config) as sess:
                self.agent = Agent(params)
                self.init_test_env(params)
                self.initialize()
                self.initialize_test(restore=restore, sess=sess)
                self.test_rollouts = 50
                self.test(sess, beam=True, print_paths=True, save_model=False)
