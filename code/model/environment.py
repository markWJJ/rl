...
import numpy as np
from data.feed_data import RelationEntityBatcher, OnlineRelationEntityBatcher
...
class env(object):
    def __init__(self, params, mode='train'):
        ...
        if mode == 'train':
            ...
            self.batcher = OnlineRelationEntityBatcher(input_triple="", 
                                         batch_size=params['batch_size'], 
                                         entity_vocab=params['entity_vocab'], 
                                         relation_vocab=params['relation_vocab'])
    ...
    def get_episodes(self, input_triple=None):
        ...
        if self.mode == 'train':
            ...
            store, store_all_correct = self.batcher.create_triple_store(input_triple)
            self.total_no_examples = store.shape[0]
            for data in self.batcher.yield_next_batch_test(store, store_all_correct):
                if data == None:
                    return
                yield Episode(self.grapher, data, params)
