...
class Minerva_Triple_Api(object):
    ...
    def initialize_inference(self, restore=None, sess=None):
        ...
        with tf.variable_scope("policy_steps_unroll") as scope:
            ...
            self.inference_loss, inference_state, self.inference_logits, self.inference_action_idx, self.chosen_relation = self.agent.step(
            ...
            self.inference_state = tf.stack(inference_state)
    ...
    def inference_api(self, params, sess, save_path):
        self.initialize_inference_env(params)
        self.initialize_inference(restore=save_path, sess=sess)
        fetches, feeds, feed_dict = self.gpu_io_setup()
        paths = self.inference(sess, beam=True, print_paths=False)
        return paths
    ...
    def inference(self, sess, beam=False, print_paths=False, save_model = True, auc = False):
        batch_counter = 0
        paths = defaultdict(list)
        feed_dict = {}
        auc = 0

        total_examples = self.inference_environment.total_no_examples
        for episode in tqdm(self.inference_environment.get_episodes()):
            batch_counter += 1
            ...
            beam_probs = np.zeros((temp_batch_size * self.inference_rollouts, 1))
            # get initial state
            state = episode.get_state()
            mem = self.agent.get_mem_shape()
            agent_mem = np.zeros((mem[0], mem[1], temp_batch_size*self.inference_rollouts, mem[3]) ).astype('float32')
            previous_relation = np.ones((temp_batch_size * self.inference_rollouts, ), dtype='int64') * self.relation_vocab[
                'DUMMY_START_RELATION']
            feed_dict[self.range_arr] = np.arange(temp_batch_size * self.inference_rollouts)
            feed_dict[self.input_path[0]] = np.zeros(temp_batch_size * self.inference_rollouts)
            
            self.entity_trajectory = []
            self.relation_trajectory = []

            self.log_probs = np.zeros((temp_batch_size*self.inference_rollouts,)) * 1.0
            ...
            for i in range(self.path_length):
                ...
                loss, agent_mem, inference_scores, inference_action_idx, chosen_relation = sess.run(
                    [ self.inference_loss, self.inference_state, self.inference_logits, self.inference_action_idx, self.chosen_relation],
                    feed_dict=feed_dict)

                if beam:
                    k = self.inference_rollouts
                    new_scores = inference_scores + beam_probs
                    if i == 0:
                ...
                state = episode(inference_action_idx)
                self.log_probs += inference_scores[np.arange(self.log_probs.shape[0]), test_action_idx]
            if beam:
            ...
            self.log_probs = np.reshape(self.log_probs, (temp_batch_size, self.inference_rollouts))
            sorted_indx = np.argsort(-self.log_probs)

            ce = episode.state['current_entities'].reshape((temp_batch_size, self.inference_rollouts))
            for b in range(temp_batch_size):
                ...
                qr = self.inference_environment.grapher.rev_relation_vocab[self.qr[b * self.inference_rollouts]]
                start_e = self.rev_entity_vocab[episode.start_entities[b * self.inference_rollouts]]
                paths[str(qr)].append(str(start_e) + "\n")
                for r in sorted_indx[b]:
                    indx = b * self.inference_rollouts + r
                    paths[str(qr)].append(
...
if __name__ == '__main__':
    ...
    if not options['load_model']:
        trainer = Minerva_Triple_Api(options)
        with tf.Session(config=config) as sess:
            sess.run(trainer.initialize())
            trainer.initialize_pretrained_embeddings(sess=sess)

            trainer.train_api(sess)
