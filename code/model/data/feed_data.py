...
class OnlineRelationEntityBatcher():
    def __init__(self, input_triple, batch_size, entity_vocab, relation_vocab, mode = "train"):
    ...
    def get_next_batch(self, store):
        yield self.yield_next_batch_test(store)
            
    def create_triple_store(self, input_triple):
        """
        input is list of dictionary that contains source entity, relation
        """
        store = []
        store_all_correct = defaultdict(set)
        if isinstance(input_triple, list):
            inputs = input_triple
        else:
            inputs = [input_triple]
            
        for item in inputs:
            e1 = self.entity_vocab[item["source_entity"]]
            r = self.relation_vocab[item["relation"]]
            store.append([e1, r, e1])
            store_all_correct[(e1, r)].add(e1)
        store = np.array(store)
        print("-------------infer------------", store)
        return store, store_all_correct

    def yield_next_batch_test(self, store, store_all_correct):
        remaining_triples = store.shape[0]
        current_idx = 0
        while True:
            if remaining_triples == 0:
                return
            ...
            if remaining_triples - self.batch_size > 0:
                ...
                batch_idx = np.arange(current_idx, store.shape[0])
                remaining_triples = 0
            batch = store[batch_idx, :]
            e1 = batch[:,0]
            r = batch[:, 1]
            e2 = batch[:, 2]
            all_e2s = []
            for i in range(e1.shape[0]):
                all_e2s.append(store_all_correct[(e1[i], r[i])])
            assert e1.shape[0] == e2.shape[0] == r.shape[0] == len(all_e2s)
            yield e1, r, e2, all_e2s
